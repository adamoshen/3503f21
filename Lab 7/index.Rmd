...
of as an interaction with oneself). We will not consider the square of `X6` since `X6` is a binary
predictor (0-1) and squaring it would result in a variable that is identical.
...
relationships are not retained. To make a prediction with the model where I squared `X3` beforehand,
I need to supply a value for `X3` and `X3sq`.
...
However, if I create the variables in the formula specification, the relationships between the
variables are retained. When predicting, I only need to supply a value for `X3`. The value of
